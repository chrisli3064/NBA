...
def get_game_logs(_name = None,year=None, playoffs=False, ask_matches=True,suffix = None,advanced = False):
    ...
    if advanced:
        ...
        selector = 'pgl_basic'
    if year or playoffs:
        r = RetriableRequest.get(f'https://www.basketball-reference.com/{suffix}/gamelog{"-advanced" if advanced else "-playoffs"if playoffs else ""}/{year if not playoffs else ""}')
        ...
        if r.status_code==200:
            ...
            if table:
                df = pd.read_html(str(table))[0]
                df.rename(columns = {df.columns[2]: 'DATE', 'Age': 'AGE', 'Tm': 'TEAM', 'Unnamed: 5': 'HOME/AWAY', 'Opp': 'OPPONENT',
        ...
        playoff_game_logs =  get_game_logs(suffix = suffix, playoffs = True)
        
        print(game_log_years)
        for season in game_log_years:
            print(f'getting 20{season[-2:]} logs',end = '\r')
            normal_game_logs = pd.concat([normal_game_logs,get_game_logs(suffix = suffix,year = f'20{season[-2:]}')])
            advanced_game_logs = pd.concat([advanced_game_logs,get_game_logs(suffix = suffix,advanced=True,year = f'20{season[-2:]}')])
        game_logs = {
